...
class Authorization {
	...
	function checklogin() {
	   if ($this->username != '' && $this->password != '' ) {
			...
			if (mysql_num_rows($result) == 1) {
				 	...
				 	$this->user_id = $uid['id'];
				 	$this->group_id = $uid['group_id'];
				 	
				 	if ($this->ext==0 && $uid['group_id'] != 1 &&  $uid['group_id'] != 2 &&  $uid['group_id'] != 7 &&  $uid['group_id'] != 8 &&  $uid['group_id'] != 9) {
				 	    return 4;
				 	}else {
				 	    return 1;
				 	}
	...
	function logout(){
		...
		mysql_query("UPDATE `users` SET `logged`='0', `extension_id`='0' WHERE `id`=$user_id");
